        ...
        authenticate("jwt-auth") {
            route("/proveedores") {
                ...
                put("/{id}") {
                    ...
                    val request = call.receive<ProveedorRequest>()
                    val ok = proveedorRepo.actualizarProveedor(id, request)
                    if (!ok) return@put call.respond(HttpStatusCode.NotFound, "Proveedor no encontrado")
                    val actualizado = proveedorRepo.getById(id) ?: return@put call.respond(
                        HttpStatusCode.InternalServerError, "No se pudo recuperar el proveedor actualizado"
                    )
                ...
                // Obtener la nota de un proveedor
                get("/{id}/nota") {
                    val id = call.parameters["id"]?.toIntOrNull()
                    if (id == null) {
                        call.respond(HttpStatusCode.BadRequest, "ID inválido")
                    ...
                    if (proveedor == null) {
                        ...
                        // Puedes devolver solo la nota o todo el proveedor. Aquí solo la nota:
                        call.respond(mapOf("nota" to proveedor.nota))
                    }
                }

// Actualizar/crear la nota
                put("/{id}/nota") {
                    val id = call.parameters["id"]?.toIntOrNull()
                    if (id == null) {
                        call.respond(HttpStatusCode.BadRequest, "ID inválido")
                    ...
                    val body = call.receive<ProveedorNotaRequest>()
                    val ok = proveedorRepo.actualizarNota(id, body.nota)
                    if (!ok) {
                        call.respond(HttpStatusCode.NotFound, "Proveedor no encontrado")
                        return@put
                    }

                    // Devuelve el proveedor actualizado para refrescar el cliente
                    val actualizado = proveedorRepo.getById(id)
                    if (actualizado != null) {
                        call.respond(HttpStatusCode.OK, actualizado)
                    } else {
                        call.respond(HttpStatusCode.InternalServerError, "No se pudo recuperar el proveedor actualizado")
                    }
                }

// Borrar la nota (dejarla en null)
                delete("/{id}/nota") {
                    val id = call.parameters["id"]?.toIntOrNull()
                    if (id == null) {
                        call.respond(HttpStatusCode.BadRequest, "ID inválido")
                    ...
                    val ok = proveedorRepo.actualizarNota(id, null)
                    if (!ok) {
                        call.respond(HttpStatusCode.NotFound, "Proveedor no encontrado")
                        return@delete
                    }
