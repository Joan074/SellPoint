...
class ProductoRepository {
    suspend fun getAll(): List<ProductoResponse> = dbQuery {
        (Productos innerJoin Categorias innerJoin Proveedores)
            .select { Productos.activo eq true }  // Filtrar solo activos
            .map { toProductoResponse(it) }
    }

    suspend fun getById(id: Int): ProductoResponse? = dbQuery {
        (Productos.innerJoin(Categorias).innerJoin(Proveedores))
            .select { (Productos.id eq id) and (Productos.activo eq true) } // También filtrar activo
            .map { toProductoResponse(it) }
            .singleOrNull()
    }
    ...
    private fun toProductoResponse(row: ResultRow): ProductoResponse {
        return ProductoResponse(
            ...
            activo = row[Productos.activo]  // agrega campo activo al data class ProductoResponse
        )
    }


    suspend fun activar(id: Int): Boolean = dbQuery {
        Productos.update({ Productos.id eq id }) {
            it[activo] = true
        } > 0
    }
